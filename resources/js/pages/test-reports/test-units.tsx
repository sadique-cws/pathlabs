    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="min-h-full bg-[#f4f7fb] p-4 md:p-6">
                <div className="mb-4">
                    <h1 className="text-3xl font-semibold text-slate-800">Manage Test Units</h1>
                    <p className="text-slate-500">Create and manage test units for laboratory measurements</p>
                </div>

                <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4">
                    <label className="relative block">
                        <Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" />
                        <input className="w-full rounded-lg border border-slate-200 py-2.5 pl-9 pr-3 text-sm" placeholder="Search test units by name..." value={search} onChange={(e) => setSearch(e.target.value)} />
                    </label>
                </div>

                <div className="mb-4 grid gap-3 md:grid-cols-2">
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                        <p className="text-slate-500">Total Test Units</p>
                        <p className="text-4xl font-bold text-slate-800">{totals.total_units}</p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                        <p className="text-slate-500">Units Available</p>
                        <p className="text-4xl font-bold text-slate-800">{totals.available_units}</p>
                    </div>
                </div>
