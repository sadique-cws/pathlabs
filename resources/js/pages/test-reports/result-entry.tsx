        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="min-h-full bg-slate-50/80 p-0">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
                    <div className=" border-b border-r border-slate-200 bg-white p-3 sm:p-4 transition-all">
                    ...
                    <div className="col-span-2 sm:col-span-1 border-b border-slate-200 bg-white p-3 sm:p-4 transition-all">
                ...
                <div className="border-b border-slate-200 bg-white p-4 shadow-sm">
                    <div className="flex gap-4">
                        <form onSubmit={handleSearch} className="relative flex-1">
                            ...
                            <input
                                className="h-9 w-full  border border-slate-200 bg-slate-50/50 pl-10 pr-3 text-sm outline-none transition focus:border-[#147da2] focus:bg-white focus:ring-1 focus:ring-[#147da2]/20"
                        ...
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <select
                                className="h-9 w-full  border border-slate-200 px-3 text-sm outline-none transition focus:border-[#147da2]"
                                value={filters.date}
                                ...
                                {paginated.map((row) => (
                                    <tr key={row.id} className="border-b border-slate-100 text-sm hover:bg-slate-50 transition-colors text-slate-700">
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <span className="mt-0.5 inline-block bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-600">{row.department}</span>
                                        </td>
                                        <td className="px-4 py-3">{row.bill_date}</td>
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex  px-2 py-0.5 text-xs font-semibold ${statusBadge[row.status] ?? 'bg-slate-100 text-slate-600'}`}>
                                                {row.status}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <Link href={`/lab/test-reports/result-entry/${row.id}`} className=" bg-[#147da2] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#106385]">
